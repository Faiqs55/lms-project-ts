  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isCacheExist) {
        ...
        await redis.set(courseId?.toString() || "", JSON.stringify(course), "EX", 604800); // 7days
...
export const addAnwser = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const deleteCourse = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await redis.del(id?.toString() as string);
      ...
      const response = await axios.post(
        ...
        { ttl: 300 },
        {
          headers: {
            ...
            Authorization: `Apisecret ${process.env.VDOCYPHER_API_SECRET}`,
          },
        }
      );
