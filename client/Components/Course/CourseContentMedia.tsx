...
const CourseContentMedia = ({
  ...
  return (
    <div className="w-[95%] 800px:w-[86%] py-4 m-auto">
      ...
      <br />
      {activeBar === 0 && (
        <p className={`text-[18px] ${data[activeVideo]?.links.length < 2 ? "py-10" : "py-0"} whitespace-pre-line mb-3 dark:text-white text-black`}>
      ...
      {activeBar === 1 && (
        <div className="">
          {data[activeVideo]?.links.map((item: any, index: number) => (
            <div className={`mb-5  ${data[activeVideo]?.links.length < 2 ? "py-10" : "py-0"}`} key={index}>
      ...
      <div className="my-4">
        ...
        <div className="w-full flex">
          ...
          <span className="pl-1 mt-[-4px] cursor-pointer text-[#000000b8] dark:text-[#ffffff83]">
            {item.questionReply.length}
          </span>
        </div>

        {replyActive && questionId === item._id &&  (
          <>
            {item.questionReply.map((item: any) => (
              <div className="w-full flex 800px:ml-16 my-5 text-black dark:text-white" key={item.createdAt}>
                <div>
                  <Image
                    src={
                      item.user.avatar
